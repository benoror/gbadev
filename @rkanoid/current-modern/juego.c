...
void FormateaDec2(u16 num, u16 cadnum[2])
{
	cadnum[0] = (num / 10) % 10;
	cadnum[1] = num % 10;
}

void FormateaDec7(u32 num, u16 cadnum[7])
{
	u16 i;

	for(i=0;i<7;i++) cadnum[i] = 20;
...
void RefreshBackground(u16 nivel, boolean mesias)
{
	u16 x,y;

	RefreshBlockMap(nivel);

	if(mesias==TRUE) ApplyShieldFloor();
...
void SetBallSprite(u16 x, u16 y, u16 rastro, u16 direccion, u16 tipoBola)
{
	u16 i;
...
void UpdateBonusSprite(u16 x, u16 y, u16 tipo, u16 num)
{
...
void UpdateLifeDisplay(u16 num)
{
	...
	for(i=0;i<10;i++) cadnum[i] = 20;
	FormateaDec2(num, cadnum);
...
void SetPaddleSprite(u16 x, u16 y, u16 long_r, u16 PLAYER)
{
	if(long_r==FALSE)
	{
...
void RunServeState(u16 rx, u16 ry, u16 long_r, u16 PLAYER, u32 ScoreCount)
{
	u16 boton,bx = rx+13, incRaqueta;

	SetPaddleSprite(rx, ry, long_r, PLAYER);
	SetBallSprite(bx,ry-ALTO_BOLA,FALSE,0,BOLA1);
	boton = (joypad());
	while ((boton & J_A))
	{
	boton = (joypad());
...
void RunGame(u16 PLAYERS)
{
	...
	RefreshBackground(nivel,mesias);
	ClearOam();
	Wait(100);	//Espera 3 segundos
	fade_in(30, 1, 0);

	InitializeBlockProperties(nivel);
	UpdateLevelDisplay(nivel);
	...
	if(jugando==TRUE) RunServeState(rx,ry,long_r,PLAYER,ScoreCount);
	...
	for (i=0 ; i <= 108 ; i++)
		if(L[i].activoBonus == TRUE)	
		{
			if(L[i].BONUS!=NO_BONUS)
				{
					UpdateBonusSprite(L[i].bonX,L[i].bonY,L[i].BONUS,L[i].bonN);
					L[i].bonY++;
					if(L[i].bonX+16>=rx && L[i].bonX<=rx+ANCHO_RAQUETA && L[i].bonY>=ry && L[i].bonY<=ry+ALTO_RAQUETA)
					{
						if(L[i].BONUS==COMET_BONUS)
							...
							UpdateBonusSprite(240+8,160+8,L[i].BONUS,L[i].bonN);
							}
		...
		SetBallSprite(bx, by, rastro, direccion, tipoBola);
		UpdateScoreDisplay(ScoreCount);

		if(LifeCount>99) LifeCount=99;
		UpdateLifeDisplay(LifeCount);

		if(IsLevelCleared()==TRUE) {nivel++; goto GANA_NIVEL;}
		RefreshBackground(nivel,mesias);
