...
void Ciclo_Movimiento_prsj1(int x, int y)
{
...
while(DreamSoft_Never_Die)
{
	...
	if(x>240) {x=0-w_prsj1;}
	if(x<0-w_prsj1) {x=240;}
	if(y>160) {y=0-h_prsj1;}
	if(y<0-h_prsj1) {y=160;}

	if(boton) {

		if(frame>7) frame = 0;

		Muestra_prsj1_XY(x,y,frames[frame], lado, estilo);
		...
		if (!(boton & J_DOWN))
		{ if(cambioframe==longitud){frame++; cambioframe=0;} y+=vel; cambioframe++; lado=down;}
		if (!(boton & J_UP))
		{ if(cambioframe==longitud){frame++; cambioframe=0;}; y-=vel; cambioframe++; lado=up;}
		if (!(boton & J_RIGHT))
		{ if(cambioframe==longitud){frame++; cambioframe=0;} x+=vel; cambioframe++; lado=right;}
		if (!(boton & J_LEFT))
		{ if(cambioframe==longitud){frame++; cambioframe=0;} x-=vel; cambioframe++; lado=left;}
