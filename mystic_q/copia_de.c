...
u16 superficie(u16 _x, u16 _y)
{
	return (MyMap[(_y+7)/8][(_x+4)/8]==0);
}
...
while(DreamSoft_Never_Die) {
	...
	if ( x != new_x && superficie(new_x,y) ) x = new_x;
	if ( y != new_y && superficie(x,new_y) ) y = new_y;
